...
# Modifications made by Cesar del Aguila/cdao2000 are licensed under the MIT License.
...
def DataCleaningTool(df: pd.DataFrame) -> str:
    return f"""
You are a data cleaning assistant.

Given a pandas DataFrame `df` with columns: {', '.join(df.columns)},
perform the following cleaning steps:


1.	Standarize by replacing spaces in column names with underscores and convert all characters to lowercase for consistency.
2.	Trim extra spaces in all string columns.
3.	Standardize text casing: capitalize names and convert other strings to lower case if appropriate.
4.	Fix inconsistent date formats into YYYY-MM-DD.
5.	Convert string numbers to numeric types and booleans where possible.
6.	Remove special characters from text fields (e.g., $, %, #, emojis).
7.	Replace/standardize common abbreviations (e.g., 'NY', 'N.Y.' → 'New York').
8.	Correct known typos (e.g., 'emial' → 'email').
9.	Fill missing values with 'Unknown' for strings, 0 for numbers.
10.	Standardize categorical values (e.g., 'M', 'male' → 'Male').
11.	Split 'Full Name' into 'First Name' and 'Last Name' if such a column exists.
12.	Combine 'Area Code' and 'Phone Number' into a single cleaned field.
13.	Format numeric fields: remove commas, set 2 decimal places if needed.
14.	Validate and correct email and phone formats.
15.	Remove duplicate rows.
16.	Recode values for clarity (e.g., 0/1 → No/Yes).
17.	Enforce consistent units (e.g., '5ft' → '152.4cm').
18.	Strip HTML or markup from any text fields.

Return the cleaned DataFrame and assign it to a variable called `result`.
Only return Python code using pandas in a single code block.
"""

def DataCleaningAgent(df: pd.DataFrame):
    prompt = DataCleaningTool(df)
    try:
        response = client.chat.completions.create(
            model="nvidia/llama-3.1-nemotron-ultra-253b-v1",
            messages=[
                {"role": "system", "content": "You are a data cleaning expert using pandas. Your response must contain ONLY a properly-closed ```python code block, no extra text."},
                {"role": "user", "content": prompt}
            ],
            temperature=0.2,
            max_tokens=1024
        )
        return extract_first_code_block(response.choices[0].message.content)
    except Exception as exc:
        return f"Error generating cleaning code: {exc}"
